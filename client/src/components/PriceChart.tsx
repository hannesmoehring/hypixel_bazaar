...
export function PriceChart({ product }: { product: string }) {
  const { data, isLoading, isError } = useQuery<Row[]>({
    ...
    queryFn: () => fetchJson<Row[]>(`http://localhost:8000/api/ticker?product=${product}`),
      ...
      <CardContent className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            <YAxis />
            <Tooltip />
            <Line dataKey="buyInstantPrice" dot={false} strokeWidth={2} />
            <Line dataKey="sellInstantPrice" dot={false} strokeDasharray="4 2" />
          </LineChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
